...
public class MyPathOverlay extends PathOverlay {
	...
	public MyPathOverlay(final int color, final ResourceProxy resourceProxy) {
		this(color, 2.0f, resourceProxy);
	}

	public MyPathOverlay(final int color, final float width, final ResourceProxy resourceProxy) {
	...
	protected void draw(final Canvas canvas, final MapView mapView, final boolean shadow) {
		...
		while (this.mPointsPrecomputed < size) {
			...
			proj.toProjectedPixels(pt.x, pt.y, pt);
		...
		Point screenPoint0; // points on screen
		Point screenPoint1;

		projectedPoint0 = this.mPoints.get(size - 1);
		screenPoint0 = proj.toPixelsFromProjected(projectedPoint0, this.mTempPoint1);

		// reset bounds
		this.mLineBounds.set(projectedPoint0.x, projectedPoint0.y, projectedPoint0.x,
				projectedPoint0.y);
		...
		for (int i = size - 2; i >= 0; i--) {
			...
			screenPoint1 = proj.toPixelsFromProjected(projectedPoint1, this.mTempPoint2);

			// skip this point, too close to previous point (< 1 px)
		...
		final BoundingBoxE6 bbox = proj.getBoundingBox();
		proj.toProjectedPixels(bbox.getLatNorthE6(), bbox.getLonWestE6(), topLeft);
		proj.toProjectedPixels(bbox.getLatSouthE6(), bbox.getLonEastE6(), bottomRight);
		final Rect bounds = new Rect(topLeft.x, topLeft.y, bottomRight.x, bottomRight.y);
		
		if (Rect.intersects(mLineBounds, bounds)) {
			canvas.drawPath(this.mPath, this.mPaint);
		} else {
//			Log.w("intersects", Rect.intersects(mLineBounds, bounds) + "\n" + mLineBounds + "\n" + bounds);
		}
	}
}
